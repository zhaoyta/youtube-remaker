#!/usr/bin/env python3
"""运行前检查 ffmpeg / edge-tts / pillow。缺了就退出。"""
...
INSTALL_HINT = {
    ...
    "pillow": "bash scripts/setup_venv.sh",
    "python3.11-venv": "bash scripts/uv_run.sh <脚本>",
}
...
def report(missing: list[str]) -> None:
    ...
    status = {
        ...
        "pillow": "pillow" not in missing,
    }
    print(f"[deps] Python {sys.version.split()[0]}  prefix={sys.prefix}", flush=True)
